...
{
    public static String TAG = "FT311UARTInterface";
    private static final String ACTION_USB_PERMISSION =    "com.randi.myapplicationemptyactivity.USB_PERMISSION";
    ...
    /*prepare the baud rate buffer*/
        writeusbdata[0] = (byte) (baud & 0xFF);
        writeusbdata[1] = (byte) ((baud >> 8) & 0xFF);
        writeusbdata[2] = (byte) ((baud >> 16) & 0xFF);
        writeusbdata[3] = (byte) ((baud >> 24) & 0xFF);

        Log.d(TAG, "SetConfig: " + baud);

        Log.d(TAG, "SetConfig: " + (writeusbdata[0] & 0xff));
        Log.d(TAG, "SetConfig: " + (writeusbdata[1] & 0xff));
        Log.d(TAG, "SetConfig: " + (writeusbdata[2] & 0xff));
        Log.d(TAG, "SetConfig: " + (writeusbdata[3] & 0xff));
        ...
        public void run()
        {
            while(READ_ENABLE == true)
            {
                ...
                try
                {
                    if(instream != null)
                    {
                        //Log.d(TAG, "run: before instream");
                        readcount = instream.read(usbdata,0,1024);
                        //Log.d(TAG, "run: after instream");
                        if(readcount > 0)
                        {
                            //Log.d(TAG, "run: " + readcount);
